...
import {
    runBeforeInvokeHooks,
    runAfterInvokeHooks,
} from './lib/invoke-hooks';
import { invokeHookPlugins } from './lib/plugins/index';
...
async function processMessage(dbMsg: DbMessage): Promise<void> {
    try {
        ...
        const beforeHookResult = await runBeforeInvokeHooks(invokeHookPlugins, {
            workspacePath,
            agentId,
            messageData,
            message,
        ...
        message = beforeHookResult.message;
        ...
        await runAfterInvokeHooks(invokeHookPlugins, {
            workspacePath,
            agentId,
            messageId,
            message,
